...
while not os.path.isfile(filename):
	...
	print(subreddit)
	for x in helper.get_reddit_redgifs_with_curl_cffi(subreddit=subreddit, limit=10, time_period="day"): #reddit.subreddit(subreddit).top(time_filter='day',limit=25):
		#print(x.url)
		if 'redgifs' in x['video_url'] & (str(x['title']) not in all_titles_ever): #('redgifs' in x.url) & (str(x.title) not in all_titles_ever):
			url = x['video_url']
			...
			break
		elif ('redgifs' in x['video_url']) & (str(x['title']) in todays_alreadysent_list):
