...
def create_caption_non_MA(long_name, short_name):
    potential_caps = ['just {} doing {} things'.format(short_name, short_name), '{} being {}'.format(short_name, short_name), 
            ...
            'Describe {} with one {}'.format(random.choice([short_name, long_name]), random.choice(['word', 'emoji']))]
    rchoice = random.choice([1,2,3,4,5,6,7])
    print(rchoice)
    if (rchoice==1):
    ...
    return post_cap
def get_tiktok_video_urls_retry(username, all_urls_ever_input, bad_urls_input):
    ...
    video_urls = []
    all_urls_ever_output = all_urls_ever_input
    retry_count = range(10)
    for round in retry_count:
        print('tik tok ping attempt: ', round, flush=True)
        try: 
            ...
            print('Issue with yt_dlp. Renewing IP')
            renew_tor_ip()
            continue
        if len(video_urls)==0:
            #renew_tor_ip()
...
def get_tiktok_video_urls(username, all_urls_ever_input, bad_urls_input):
    ...
    video_urls = []
    all_urls_ever_output = all_urls_ever_input
    if username=='sophieraiin':
        historical_tts_list = historical_tts.sr_tts
    elif username=='aishah':
        historical_tts_list = historical_tts.as_tts
    elif username=='realalinnarosee':
        historical_tts_list = historical_tts.ar_tts
    elif username=='leximarvell':
        historical_tts_list = historical_tts.lm_tts
    elif username=='kirstentoosweet':
        historical_tts_list = historical_tts.ks_tts
    elif username=='ema_bb0':
        historical_tts_list = historical_tts.em_tts
    elif username=='sommerray':
        historical_tts_list = historical_tts.somr_tts
    elif username=='francety':
        historical_tts_list = historical_tts.fj_tts
    elif username=='miakhalifa':
        historical_tts_list = historical_tts.mk_tts
    elif username=='reidyourmind':
        historical_tts_list = historical_tts.rr_tts
    elif username=='breckiehill':
        historical_tts_list = historical_tts.bh_tts
    ...
    with yt_dlp.YoutubeDL(ydl_opts) as ydl:
        try:
            ...
            print(f"An error occurred with yt_dlp: {e}")
            print(f"using historical tts")
            renew_tor_ip() 
            video_urls = historical_tts_list
...
def create_caption_tt(long_name, short_name):
    potential_caps = ['just {} {} {}'.format(short_name, random.choice(['being', 'doing']), short_name), 
                      '{}?'.format(random.choice(['smash or pass','kiss, spank, or hug','girlfriend or wife','girlfriend, wifey, or mistress', 'fmk'])),
                      'rate {} 1-{}'.format(random.choice([long_name, short_name]), random.choice(['10', '69', '100'])),
                      "You're {} if you find {} attractive".format(random.choice(['sick','crazy','nuts']), random.choice([short_name,long_name])),
            "Stop {} and {} {}".format(random.choice(['scrolling', "what you're doing", 'everything']), random.choice(['stare at', 'appreciate', 'admire']), long_name), 
            "{} needs some {}".format(short_name, random.choice(['attention','views','screen time'])), 
            "What's your favorite {} {}?".format(long_name, random.choice(['move','expression','outfit','fit', 'feature'])), 
            'All {} all the time!'.format(random.choice([short_name, long_name])), 
            "Go {} one of {}'s posts".format(random.choice(['like','retweet','share','comment on']), short_name), 
            '{} is encouraged'.format(random.choice(['Obsession','Falling in love', 'Attention'])),
            "{} or {}?".format(random.choice([short_name, long_name]), random.choice(['2 million dollars','gta7','a ps6','1 billion dollars', 'a lambo', 'free rent'])),
            'Describe {} in one {}'.format(random.choice([short_name, long_name]), random.choice(['word', 'emoji', 'saying', 'phrase'])),
            "Is {} {}?".format(random.choice([short_name, long_name]), random.choice(["your type", "deserving of the hype","a 10", "one of your hall passes"])),
            "Are you {} {}?".format(random.choice(['following', 'subbed to', 'hooked on','thinking about']), random.choice([short_name, long_name])),
            "Rate {}'s {} from 1 to {}".format(random.choice([short_name, long_name]), random.choice(['vibe', 'energy']), random.choice(['10','69','100'])),
            "Quick: drop a {} for {}".format(random.choice(imojis_list), random.choice([short_name, long_name])),
            "What is {} {} right here? {}".format(random.choice([short_name, long_name]), random.choice(['thinking about', 'doing']), random.choice(['','Wrong answers only'])),
            "What {} describes {} best?".format(random.choice(['word', 'emoji', 'saying', 'phrase']), random.choice([short_name, long_name])),
            "wyd if {} wants to {}?".format(random.choice([short_name, long_name]), random.choice(['move in','go on a date','look at your phone'])),
            "How would you describe {}'s impact on {}?".format(random.choice([short_name, long_name]), random.choice(['society','your life','the world','Twitter'])),
            "Is {} {}?".format(random.choice([short_name, long_name]), random.choice(['your lockscreen', 'living rent-free in your head', 'getting prettier'])),
            "How will {} {} {}?".format(random.choice(['history','the world','future generations']), random.choice(['remember','look back on','think about']), random.choice([short_name, long_name]))
        ]
