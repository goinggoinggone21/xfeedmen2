...
def get_tiktok_video_urls(username, all_urls_ever_input, bad_urls_input):
    ...
    if username=='sophieraiin':
        ...
        historical_tts_list = historical_tts.mk_tts_tts
...
def create_caption_tt(long_name, short_name):
    potential_caps = ['just {} {} {}'.format(short_name, random.choice(['being', 'doing']), short_name), 
                      '{}?'.format(random.choice(['smash or pass','kiss, spank, or hug','girlfriend or wife'])),
                      'rate {} 1-10'.format(random.choice([long_name, short_name])),
                      "You are {} if you find {} attractive".format(random.choice(['sick','crazy']), random.choice([short_name,long_name])),
            ...
            "What's your favorite {} {}?".format(long_name, random.choice(['move','expression','outfit','fit'])), 
            'All {} all the time!'.format(random.choice([short_name, long_name])), 
            "Go {} one of {}'s posts".format(random.choice(['like','retweet','share','comment on']), short_name), 
            '{} is encouraged'.format(random.choice(['Obsession','Falling in love', 'Attention'])),
            "{} or {}?".format(random.choice([short_name, long_name]), random.choice(['2 million dollars','gta6','a ps6','1 billion dollars', 'a lambo'])),
