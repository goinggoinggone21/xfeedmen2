...
all_urls_fn = 'all_sm_feed_urls_ever.ob'
try:
...
try:
	#Load TikToks
	tiktoks, all_urls_ever = helper.get_tiktok_video_urls(username='skylarmaexoxoxo', 
		all_urls_ever_input=all_urls_ever, bad_urls_input=bad_urls)

	filename = 'to_upload_sm.mp4'
	...
	for tiktok_submission in tiktoks:
		...
		if (str(submission_url) not in all_urls_ever):
			...
			continue
	tweet_title_final = helper.create_caption_tt(long_name='Skylar Mae',short_name='Skylar') #random.choice(helper.imojis_list)*3 
