...
all_urls_fn = 'all_mm_feed_urls_ever.ob'
try:
...
#Load Reddits
#Load Reddits
reddits_with_redgif, all_urls_ever = helper.load_reddits(subreddit_input='MiaMalkova', 
	all_urls_ever_input=all_urls_ever, bad_urls_input=bad_urls)

filename = 'to_upload_mm.mp4'
...
try:
	...
	for reddit_submission in reddits_with_redgif:
		...
		if (str(submission_url) not in all_urls_ever):
			...
			continue
	tweet_title_final = helper.create_caption_non_MA(long_name='Mia Malkova',short_name='Mia') #random.choice(helper.imojis_list)*3 
